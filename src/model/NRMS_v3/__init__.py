...
class NRMS_v3(torch.nn.Module):
    ...
    def forward(self, candidate_news, clicked_news):
        ...
        clicked_news_id = torch.concat(
            [x['new_id'][:, None, :] for x in clicked_news], 
            dim = 1
        )
        clicked_news_id = torch.max(clicked_news_id, dim = -1).values
        clicked_news_vector = torch.stack(
            [self.news_encoder(x) for x in clicked_news], dim=1)
        # batch_size, word_embedding_dim
        user_vector = self.user_encoder(clicked_news_id, clicked_news_vector)
        # batch_size, 1 + K
    ...
    def get_user_vector(self, clicked_news_id, clicked_news_vector):
        """
        Args:
        ...
        """
        # batch_size, word_embedding_dim
        return self.user_encoder(clicked_news_id, clicked_news_vector)
