...
class BaseDataset_v2(Dataset):
    def __init__(self, behaviors_path, news_path):
        ...
        self.news_parsed = pd.read_table(
            news_path,
            index_col='id',
            usecols=['id'] + list(set(config.dataset_attributes['news']) - {'new_id'}),
            converters={
        ...
        self.news_parsed['new_id'] = [(x, self.news_id2int[x] + 1) for x in self.news_parsed.index]
        for attribute in config.dataset_attributes['news']:
            if attribute in {
                'category',
                'subcategory',
                ...
                self.news_parsed['new_id'] = self.news_parsed['new_id'].apply(
                    lambda x : [x[1] for _ in range(len(self.news2dict[x[0]][attribute]))]
                )
                break
        self.news2dict = self.news_parsed.to_dict('index')
