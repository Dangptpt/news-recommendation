...
class NewsDataset(Dataset):
    ...
    def __init__(self, news_path):
        ...
        self.news_id2int = {x: i for i, x in enumerate(self.news_parsed['id'].unique())}
        
        
        self.news2dict = self.news_parsed.to_dict('index')
        self.news_parsed['new_id'] = self.news_parsed.apply(
            lambda x : (x.name, self.news_id2int[x['id']] + 1),
            axis = 1
        )
